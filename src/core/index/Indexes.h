...
public:
    ...
    void debug_print_term_dict() const;

    /// @brief return how many doc contain this term
    uint32_t term_docs_num(const std::string &term) const;

    void search_field(const std::string &field_path,const std::unordered_map<string,std::vector<uint32_t>> &token_to_offsets,size_t slop,uint32_t total_doc_count,const std::unordered_map<std::string, SortIndex> *sort_indexes,std::function<void(Search::MatchContext&)> on_match) const;
...
public:
    ...
    void search_fields(const std::string &field_path,const std::unordered_map<string,std::vector<uint32_t>> &token_to_offsets,size_t slop,std::function<void(Search::MatchContext&)> on_match) const;
